jQuery(document).ready( function() {
  jQuery('.button-BENrueeg_RUE_top.ftb11,.button-BENrueeg_RUE_top.ftb12,#submit-ftb0-BENrueeg_RUE,#submit-ftb2-BENrueeg_RUE,#submit-ftb3-BENrueeg_RUE').on('click',function()  { jQuery(this).val(BENrueeg_RUE_jsParams.wait_a_little); });
});

// settings updated message
jQuery(document).ready( function($) {
	setTimeout(function(){
	    $(".benrueeg_rue_process_msg_up_db.settings_updated").slideDown(200).delay(3500).slideUp(300);
    }, 300);
});
// settings updated message
...
// benrueeg_rue_process_msg_up_db message fixed width 100%
jQuery(document).ready( function($) {
	
$(document).ready(benrueeg_rue_resizeMsgAll);
$(window).on('scroll resize',benrueeg_rue_resizeMsgAll);
function benrueeg_rue_resizeMsgAll() {
  var $ab = $('#wpadminbar');
  var isAdminBarFixed = ($ab.css('position') == 'fixed');
  var $el = $('.benrueeg_rue_process_msg_up_db.settings_updated,.benrueeg_rue_process_msg_up_db.up_process');
...
if( !isAdminBarFixed ) {
  ...
  if ($(this).scrollTop() > height){
    ...
    $el.css({'position': 'absolute', 'top': height+'px'});
  }
  } else {
	  $el.css({'position': 'fixed', 'top': height+'px'});
  }
  
// right left
var $menu = $('.auto-fold #adminmenuback');
var isMenu = ($menu.css('display') == 'none');
$el.css({'right':0,'left':0});
if( !isMenu ) {
var w1 = $('#adminmenuback').width();

if (BENrueeg_RUE_jsParams.is_rtl) {
	
$el.css({'right':w1+'px','left':0});
$('#collapse-menu').on('click.collapse-menu', function() {	
var w = $('#adminmenuback').width();
$el.css({'right':w+'px','left':0});
});

} else {
	
$el.css({'left':w1+'px','right':0});
$('#collapse-menu').on('click.collapse-menu', function() {	
var w = $('#adminmenuback').width();
$el.css({'left':w+'px','right':0});
});
